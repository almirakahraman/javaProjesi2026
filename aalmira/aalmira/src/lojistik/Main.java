...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Ornek musteriler
        Musteri almira = new Musteri(1, "Almira", new Konum(3, 4), false);      // normal sokak
        Musteri emriye = new Musteri(2, "Emriye", new Konum(6, 8), true);       // DAR sokak
        Musteri hilal = new Musteri(3, "Hilal", new Konum(1, 1), false);        // normal sokak
        ...
        try {
            drone.teslimatYap(hilal, paket3); // 2 kg < 5 kg -> basarili
        ...
        try {
            drone.teslimatYap(almira, paket4); // 7 kg >= 5 kg -> istisna
        } catch (AgirPaketException e) {
        ...
        motor.teslimatYap(hilal, paket5);
        ...
        azYakitliMotor.teslimatYap(almira, paket6); // mesafe=5, yakit=2 -> ikmal gerekir

        baslik("SENARYO 7: Paketlerin FIFO sirasiyla kuyruktan cekilmesi");
        LojistikSistemi sistem = new LojistikSistemi(logger);
        sistem.musteriEkle(almira);
        sistem.musteriEkle(hilal);
        ...
        sistem.paketiKuyrugaEkle(new Paket(4, "Paket-A", almira));
        sistem.paketiKuyrugaEkle(new Paket(6, "Paket-B", hilal));
        sistem.paketiKuyrugaEkle(new Paket(8, "Paket-C", almira));
